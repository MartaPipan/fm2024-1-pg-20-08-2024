...
-- Таблиця contracts: інформація про договори, які включають товари
CREATE TABLE contracts (
    ...
    planned_quantity INT NOT NULL CHECK (planned_quantity > 0), -- Планова кількість товару
    FOREIGN KEY (customer_id) REFERENCES customers(customer_id) ON DELETE CASCADE,
    FOREIGN KEY (product_id) REFERENCES products(product_id) ON DELETE CASCADE,
    CONSTRAINT unique_contract_number UNIQUE (contract_number)
);

-- Таблиця requests: замовлення на постачання товарів
